...
public class Utils {
    ...
    public static String getDate(long var0) {
        ...
        Calendar var2 = Calendar.getInstance(localeSpanish);
        var2.setTimeInMillis(var0);
            ...
            case CometChatConstants.CATEGORY_MESSAGE:
                if (lastMessage instanceof TextMessage) {
                    if (isLoggedInUser(lastMessage.getSender()))
                        message = " " + ((TextMessage) lastMessage).getText();
                    else
                    ...
                    if (isLoggedInUser(lastMessage.getSender())) 
                        if (lastMessage.getType().equals("audio")) {
                            message = "Has enviado un audio";

                        } else if (lastMessage.getType().equals("LOCATION")) {
                            message = "Has enviado una ubicación";

                        } else {
                            message = "Has enviado un " + lastMessage.getType();
                        }
                    else
                        if (lastMessage.getType().equals("audio")) {
                            message = "Has recibido un audio";

                        } else if (lastMessage.getType().equals("LOCATION")) {
                            message = "Has recibido una ubicación";

                        } else {
                            message = "Has recibido un " + lastMessage.getType();
                        }
                }
